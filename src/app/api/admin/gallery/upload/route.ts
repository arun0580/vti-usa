...
import {
  GALLERY_UPLOAD_DIR,
  GALLERY_UPLOAD_URL_PREFIX,
} from "@/lib/gallery-page/uploads";
...
export async function POST(request: Request) {
  ...
  const uploadDir = path.join(process.cwd(), "public", GALLERY_UPLOAD_DIR);
  await mkdir(uploadDir, { recursive: true });

  const buffer = Buffer.from(await file.arrayBuffer());
  await writeFile(path.join(uploadDir, filename), buffer);

  const url = `${GALLERY_UPLOAD_URL_PREFIX}/${filename}`;
  return NextResponse.json({
    success: true,
    data: { url, filename },
  });
