...
import { revalidateCmsPage } from "@/lib/page-cms/cache";
...
export async function GET() {
  ...
  const upstream = await proxyToAdminApi("/api/admin/pages/reseller-portal", undefined, token);
...
export async function PUT(request: Request) {
  ...
  const upstream = await proxyToAdminApi(
    "/api/admin/pages/reseller-portal",
  ...
  if (!upstream.ok || !data?.success) {
    return NextResponse.json(data ?? { success: false, error: "Save failed" }, {
      status: upstream.status || 500,
    });
  }

  revalidateCmsPage("reseller-portal");
  return NextResponse.json(data);
}
