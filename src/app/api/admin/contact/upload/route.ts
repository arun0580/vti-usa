...
import {
  CONTACT_UPLOAD_DIR,
  CONTACT_UPLOAD_URL_PREFIX,
} from "@/lib/contact-page/uploads";
...
export async function POST(request: Request) {
  ...
  const uploadDir = path.join(process.cwd(), "public", CONTACT_UPLOAD_DIR);
  await mkdir(uploadDir, { recursive: true });

  const buffer = Buffer.from(await file.arrayBuffer());
  await writeFile(path.join(uploadDir, filename), buffer);

  const url = `${CONTACT_UPLOAD_URL_PREFIX}/${filename}`;
  return NextResponse.json({
    success: true,
    data: { url, filename },
  });
